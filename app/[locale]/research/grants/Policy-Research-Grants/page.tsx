...
export default function PolicyResearchGrantsPage() {
  const t = useTranslations("PRGPage");
    ...
    <>
      ...
      <main className="min-h-screen bg-white font-poppins">
        ...
        <RailSection tint title={t("portalTitle")}>
          <div className="space-y-4 font-poppins text-[15px] leading-[1.9] text-[#1D2D44]/65 font-light">
            <motion.p {...fadeUp(0.05)}>{t("portalBody1")}</motion.p>
            <motion.p {...fadeUp(0.1)}>
              <Link
        ...
        <RailSection title={t("attachmentsTitle")}>
          ...
          <div className="mt-6 divide-y divide-[#1D2D44]/[0.08]">
            {attachmentItems.map((doc, i) => {
              const isActive = Boolean(doc.href);
              const Item = isActive ? motion.a : motion.div;
              return (
                <Item
                  key={doc.label}
                  {...(isActive
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <span
                      className={`font-poppins text-[14px] font-medium text-[#1D2D44] transition-colors ${
                        isActive ? "group-hover:text-[#EC601B]" : ""
                      }`}
                    >
                      {doc.label}
                    </span>
